...
UPDATE_SERVICE_BASE_URL = "https://updates.iotplatform.io"
...
class TBUpdater:
    def __init__(self):
        ...
        try:
            distribution_version = metadata_version('iotplatform_gateway')
    ...
    def update(self):
        if self.__version["latest_version"] != self.__version["current_version"]:
            result = TBUtility.install_package("iotplatform-gateway", self.__version["latest_version"])
        else:
