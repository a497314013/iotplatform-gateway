...
from iotplatform_gateway.connectors.rest.backward_compatibility_adapter import BackwardCompatibilityAdapter
    ...
    def load_endpoints(self):
        ...
        if len(self.__requests_config.get('attributeRequests', [])):
            while self.__gateway.tb_client is None and not hasattr(self.__gateway.tb_client, 'client'):
                self.__log.info('Waiting for IOTPlatform client to be initialized...')
                sleep(1)
        ...
        if self.__server_config.get('SSL', False):
            if not self.__server_config.get('security'):
                if not os.path.exists('domain_srv.crt'):
                    from iotplatform_gateway.connectors.rest.ssl_generator import SSLGenerator
